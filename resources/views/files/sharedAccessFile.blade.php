@extends(Auth::check() ? 'layouts.layout' : 'layouts.guestLayouts')
                    ...
                    <div class="card-header">
                        @if (Auth::check())
                            @if ($file->pin && $userId != $file->user_id)
                                <form action="{{ route ('download.SharedAccess', ['id' => $file->id]) }}" method="post" class="float-left">
                                    @csrf
                            ...
                            @else
                                <a href="{{ route ('download.SharedAccess', ['id' => $file->id]) }}" class="btn btn-success btn-sm float-left mr-1">
                                    <i class="fas fa-arrow-down"></i> Cкачать
                                </a>
                            @endif
                            @if ($userRole == 'administrator' || $userId == $file->user_id)
                                ...
                                {{ route('file.sharedAccess', ['file' => $file->id]) }}</br>
